(()=>{
 ...
 function renderPeople(){
  ...
  const rows=data.users.filter(u=>(u.name+' '+u.email).toLowerCase().includes(q)&&(!org||(u.organisation_id||'unassigned')===org)&&(!status||u.status===status));
  $('people').innerHTML=rows.map(u=>`<tr><td>${esc(u.name)}<small>${esc(u.email)}</small><small>${u.email_confirmed_at?'Email confirmed':'Email not confirmed'}</small></td><td>${esc(u.organisation||'Unassigned')}</td><td><span class="chip">${esc(u.status)}</span><small>${esc(u.tier)} · ${esc(u.role)}</small><small>Expires: ${u.access_expires_at?esc(stamp(u.access_expires_at)):'No expiry set'}</small></td><td>${esc(stamp(u.last_activity))}</td><td>${Number(u.tool_opens)}</td><td>${Number(u.analysis_requests)}</td></tr>`).join('')||empty(6,'No matching accounts.');
 }
 ...
 function render(){
  ...
  $('orgFilter').innerHTML='<option value="">All organisations</option>'+data.organisations.map(o=>`<option value="${esc(o.id||'unassigned')}">${esc(o.name)}</option>`).join('');$('orgFilter').value=selected;
  renderPeople();
 ...
 async function load(){
  ...
  try{
   ...
   data=result;render();$('content').hidden=false;$('export').disabled=false;$('status').textContent='Updated '+stamp(result.generated_at)+' · Account and organisation details are current; usage follows the selected date.';
