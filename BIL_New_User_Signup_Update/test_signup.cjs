...
async function submit(t){t.els.signupEmail.value='new@example.test';t.els.signupPassword.value='Example-only-123';t.els.signupConfirm.value='Example-only-123';await t.els.signupForm.onsubmit({preventDefault(){}})}
(async()=>{
 let t=setup();await submit(t);assert.equal(t.calls.length,1);assert.equal(t.calls[0].options.emailRedirectTo,'https://example.test/login.html?next=%2Fcases.html');assert.equal(t.redirects.length,0);assert.match(t.els.status.textContent,/Check your email/);assert.equal(t.els.signupPassword.value,'');assert.equal(t.els.createAccount.disabled,false);
